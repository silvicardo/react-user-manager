  ...
  return createReducer(initialState, {
    ...
    SET_USER_TO_BE_FRIEND: (state, action) => {
      ...
      if (alreadyStoredFriend && alreadyStoredFriend.friendshipId) {
        state.next.removingFriendshipsIds = state.next.removingFriendshipsIds.filter(
          (friendshipId) => friendshipId !== alreadyStoredFriend.friendshipId
        );
      }
    },
...
export default function useUserEditor(id = null) {
  //db data
  const [storedUsername, storedFriends, storedUnrelatedUsers, isFetchingUserData, userApiError] = useUser(id);
  ...
  const onNextUsernameChange = (value) =>
    dispatch({
