...
export const FriendshipsEditor = ({
  className = "",
  userId = null,
  notYetFriends,
  friends,
      ...
      <div className={"border border-dark rounded p-3"}>
        ...
        <div className={"body pt-3"}>
          <div className={`pick-view ${activeView !== "pick" ? "d-none" : ""}`}>
            <SelectableFriendsList users={notYetFriends} onPickFriendClick={onPickFriendClick} />
          </div>
          <div className={`edit-view ${activeView !== "edit" ? "d-none" : ""}`}>
            <h3 className={friends.length > 0 ? "d-none" : ""}>{lang.users.noFriends}</h3>
            <EditableFriendsList friends={friends} onUnfriendClick={onUnfriendClick} />
          </div>
        </div>
      </div>
    </div>
